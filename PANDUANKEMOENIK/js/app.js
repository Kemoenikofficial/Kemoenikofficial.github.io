...
function initApp() {
    try {
        ...
        if (expiry && Date.now() > parseInt(expiry)) {
            // Auto-renew expiry daripada logout pengguna
            localStorage.setItem('kemoenik_expiry', String(Date.now() + 7 * 24 * 60 * 60 * 1000));
        }
...
function go(page) {
    ...
    try { gtag('event', 'page_view', { page_title: page }); } catch(e) {}
}
...
function hitungProgressMinggu() {
    ...
    for (var i = 0; i < 7; i++) {
        ...
        if (appState.misiChecked && appState.misiChecked[key]) {
            terpenuhi++;
        }
    }
    var persen = Math.round((terpenuhi / totalHari) * 100);
    return { terpenuhi: terpenuhi, total: totalHari, persen: persen };
}

function generateKalenderMini() {
    var hari = ['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'];
    var today = new Date();
    var html = '<div style="display:flex; gap:6px; justify-content:center;">';
    for (var i = 6; i >= 0; i--) {
        var d = new Date(today);
        d.setDate(today.getDate() - i);
        var key = 'olahraga_' + d.toDateString();
        var checked = appState.misiChecked && appState.misiChecked[key];
        var dayName = hari[d.getDay()];
        html += `<div style="display:flex; flex-direction:column; align-items:center;">
            <span style="font-size:10px; color:var(--text3);">${dayName}</span>
            <div style="width:20px; height:20px; border-radius:50%; background:${checked ? 'var(--green)' : 'var(--border)'}; margin-top:2px;"></div>
        </div>`;
    }
    html += '</div>';
    return html;
...
function pilihAfirmasi() {
    var list = [
        "Luar biasa! Tubuhmu semakin bugar! 💪",
        "Konsisten adalah kunci! Pertahankan! 🔥",
        "Olahraga hari ini = lemak terbakar!",
        "Kamu hebat! Jaga terus ritmenya!",
        "Sedikit lebih baik setiap hari!",
        "Istirahat yang cukup juga bagian dari progress."
    ];
...
function renderHomeJadwal() {
    ...
    try {
        ...
        var manfaat = todayData.manfaat || 'Meningkatkan metabolisme, membakar kalori.';
        var isRest = akt.indexOf('Istirahat') !== -1 || akt.indexOf('recovery') !== -1;
        var todayKey = 'olahraga_' + new Date().toDateString();
        var checked = appState.misiChecked && appState.misiChecked[todayKey];

        var streak = hitungStreak();
        var progress = hitungProgressMinggu();

        var html = `
            <div class="olahraga-card" style="background:var(--card); border-radius:20px; padding:20px; box-shadow:var(--shadow-sm); margin-bottom:14px;">
                <div class="streak-badge" style="display:flex; align-items:center; gap:6px; background:rgba(31,77,58,0.1); color:var(--green2); padding:6px 12px; border-radius:30px; width:fit-content; margin-bottom:14px;">
                    <span style="font-size:16px;">🔥</span>
                    <span style="font-weight:600; font-size:13px;">${streak} hari berturut-turut!</span>
                </div>
                <div class="progress-minggu" style="margin-bottom:14px;">
                    <div style="display:flex; justify-content:space-between; font-size:12px; color:var(--text3); margin-bottom:4px;">
                        ...
                        <span>${progress.terpenuhi} dari ${progress.total} hari</span>
                    </div>
                    <div style="height:6px; background:var(--border); border-radius:3px; overflow:hidden;">
                        <div style="height:100%; width:${progress.persen}%; background:linear-gradient(90deg, var(--green), var(--green2)); border-radius:3px;"></div>
                    </div>
                </div>
                <div class="kalender-mini" style="margin-bottom:14px;">
                    ${generateKalenderMini()}
                </div>
                <div class="olahraga-hari-ini" style="background:${isRest ? '#F9FAFB' : '#ECFDF5'}; border-radius:14px; padding:14px; margin-bottom:12px;">
                    <div style="display:flex; align-items:center; gap:8px; margin-bottom:6px;">
                        <span style="font-size:20px;">${isRest ? '😴' : '🏃'}</span>
                        <span style="font-weight:700; color:${isRest ? '#6B7280' : '#065F46'};">${akt}</span>
                    </div>
                    <div class="manfaat" style="font-size:12px; color:${isRest ? '#6B7280' : '#065F46'}; margin-bottom:8px;">
                        ✨ ${manfaat}
                    </div>
                    <div class="ceklis" onclick="toggleMisi('${todayKey}', this)" style="display:flex; align-items:center; gap:8px; cursor:pointer;">
                        <div class="mission-check${checked ? ' checked' : ''}" style="width:24px; height:24px; border-radius:8px; border:2px solid ${isRest ? '#9CA3AF' : '#2E6B53'}; display:flex; align-items:center; justify-content:center;">
                        ...
                        <span style="font-size:13px; color:var(--text2);">Tandai selesai</span>
                    </div>
                </div>
                <div class="afirmasi" id="afirmasi-${todayKey}" style="font-size:12px; color:var(--green2); font-style:italic; text-align:center; padding:8px; background:rgba(31,77,58,0.05); border-radius:10px;">
                    ${pilihAfirmasi()}
                </div>
            </div>
            <button onclick="openPanel('panelJadwalOlahraga')" style="width:100%; padding:12px; background:var(--white); border:1.5px solid var(--border); border-radius:14px; font-size:13px; font-weight:600; color:var(--green2); cursor:pointer;">
...
function renderMakroShortcut() {
    ...
    if (!k || !k.dietCal || !k.berat) {
        container.innerHTML = '<div style="font-size:11px; color:var(--text3); text-align:center;">Isi kalkulator<br>untuk melihat nutrisi</div>';
        return;
    }
    ...
    container.innerHTML = `
        <div style="display:flex; flex-direction:column; gap:2px;">
            <div style="display:flex; justify-content:space-between;">
                <span style="font-size:11px; color:var(--text3);">Protein</span>
                <span style="font-weight:700; color:var(--green);">${protein} g</span>
            </div>
            <div style="display:flex; justify-content:space-between;">
                <span style="font-size:11px; color:var(--text3);">Lemak</span>
                <span style="font-weight:700; color:var(--gold2);">${lemak} g</span>
            </div>
            <div style="display:flex; justify-content:space-between;">
                <span style="font-size:11px; color:var(--text3);">Karbohidrat</span>
                <span style="font-weight:700; color:#3B82F6;">${karbo} g</span>
            </div>
        </div>
    `;
}
...
async function resetKalkulator() {
    ...
    if (!wa) return;
    try {
        await DataService.saveKalkulator(wa, null);
...
function deleteEval(idx) {
    if (!confirm('Hapus catatan evaluasi ini?')) return;
...
// ========== RENDER EVALUASI HISTORY (dengan tombol hapus) ==========
function renderEvalHistory() {
    ...
    appState.evaluasi.slice().reverse().forEach(function(e, revIdx) {
        ...
        html += '<button onclick="deleteEval(' + realIdx + ')" style="padding:5px 10px;background:#FEE2E2;border:1px solid #FECACA;border-radius:6px;font-size:11px;cursor:pointer;font-family:inherit;color:#C0392B;">Hapus</button>';
        html += '</div></div>';
...
function toggleMisi(misiKey, el) {
    ...
    if (misiKey.indexOf('olahraga') === 0) {
        renderHomeJadwal();
    }
}
window.toggleMisi = toggleMisi;

// ========== FUNGSI BARU: renderStatusIndikator ==========
