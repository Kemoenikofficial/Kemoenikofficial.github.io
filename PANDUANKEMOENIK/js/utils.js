...
const AUTH_APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwcMt1x8X6ir9gGbCdY5JjPGsSMKvJA8e3RlPb2Q921xBOkglkICYqwZSgm3HTmHwf6BQ/exec';
...
// ========== FINGERPRINT DEVICE (SAMA PERSIS DENGAN aktivasi/index.html) ==========
// Dipakai khusus untuk validasi ke sistem login/register baru (checkSession),
// TIDAK menggantikan getDeviceId() lama di bawah yang dipakai fitur binding lain.
...
// ========== VALIDASI SESI KE BACKEND BARU (login/register/device-lock) ==========
// Menggantikan checkVoucherValid lama yang menembak sistem voucher lama
// (sudah tidak ada lagi di Code.gs baru).
async function checkSessionValid(username) {
  try {
    ...
    if (data.status === 'SUCCESS') {
      ...
      return { valid: false, message: 'Masa akses sudah berakhir. Silakan lakukan perpanjangan di halaman login.' };
    } else if (data.status === 'DEVICE_MISMATCH') {
      return { valid: false, message: 'Akun ini terhubung ke perangkat lain.' };
    } else if (data.status === 'INVALID') {
      return { valid: false, message: 'Sesi tidak ditemukan. Silakan login ulang.' };
    } else {
      // Status tak dikenal / server bermasalah -> jangan block user, anggap fallback
      return { valid: true, fallback: true };
    }
  } catch (e) {
    console.error('checkSessionValid error:', e);
    // Offline / gagal konek -> jangan block UI, anggap fallback
    return { valid: true, fallback: true };
  }
}
...
function openPanel(id) {
  ...
  // Cek perubahan berat saat panel kalkulator dibuka
  if (id === 'panelKalkulator') {
    setTimeout(function() {
      try {
        var k = (typeof appState !== 'undefined') ? appState.kalkulator : null;
        if (!k || !k.berat || !k.dietCal) return; // belum pernah simpan kalkulator

        // Ambil berat terakhir dari evaluasi
        var evalHistory = [];
        try {
          var wa = (typeof normalizeWA !== 'undefined')
            ? normalizeWA(appState.user && appState.user.wa || localStorage.getItem('kemoenik_wa') || '')
            : localStorage.getItem('kemoenik_wa') || '';
          var raw = localStorage.getItem('kemoenik_eval_' + wa);
          if (raw) evalHistory = JSON.parse(raw);
        } catch(e) {}

        if (evalHistory.length > 0) {
          var lastEval = evalHistory[evalHistory.length - 1];
          var beratTerakhir = parseFloat(lastEval.beratSekarang || lastEval.berat || 0);
          var beratKal = parseFloat(k.berat || 0);

          if (beratTerakhir > 0 && Math.abs(beratTerakhir - beratKal) >= 1) {
            var selisih = (beratKal - beratTerakhir).toFixed(1);
            var arah = selisih < 0 ? 'turun ↓' : 'naik ↑';
            var banner = document.getElementById('beratUpdateBanner');
            if (!banner) {
              banner = document.createElement('div');
              banner.id = 'beratUpdateBanner';
              banner.style.cssText = 'background:#FEF3C7;border:1.5px solid #F59E0B;border-radius:12px;padding:12px 14px;margin:0 0 14px 0;font-size:12px;color:#78350F;line-height:1.6;';
              var form = document.getElementById('kalkulatorForm');
              if (form) form.insertBefore(banner, form.firstChild);
            }
            banner.style.display = 'block';
            banner.innerHTML = '⚠️ <strong>Berat badanmu berubah!</strong><br>'
              + 'Data evaluasi terakhir: <strong>' + beratTerakhir + ' kg</strong> '
              + '(BB kalkulator: ' + beratKal + ' kg, ' + arah + ' ' + Math.abs(selisih) + ' kg)<br>'
              + '<span style="color:#D97706;">Disarankan hitung ulang program agar target kalori tetap akurat 🎯</span>';
          }
        }
      } catch(e) {}
    }, 200);
...
window.getAuthDeviceId = getAuthDeviceId;
